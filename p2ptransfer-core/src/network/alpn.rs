...
pub struct PauseFlag {
    ...
    /// Retain initial receivers so the channel receiver count is always >= 1,
    /// ensuring `send()` never fails with SendError even before `wait_if_paused` subscribes.
...
/// Set to 32 to keep the pipeline saturated (128 MB buffer at 4 MB chunks)
/// while maintaining responsive pause, cancellation, and progress updates.
...
pub struct TransferProtocol {
    ...
    pub event_tx: mpsc::Sender<crate::network::alpn::TransferEvent>,
...
impl ProtocolHandler for TransferProtocol {
    ...
    fn accept(&self, connection: Connection) -> Pin<Box<dyn Future<Output = Result<(), AcceptError>> + Send>> {
        ...
        Box::pin(async move {
            if let Err(e) = handle_incoming_connection(connection, pending, event_tx, output_dir).await {
...
async fn handle_incoming_connection(
    ...
    event_tx: mpsc::Sender<crate::network::alpn::TransferEvent>,
    ...
    let _ = event_tx.try_send(
        crate::network::alpn::TransferEvent::Incoming(IncomingTransferEvent {
    ...
    if !accepted {
        ...
        TRANSFER_PAUSE_FLAGS.remove(&request.request_id);
        let response = TransferResponse {
            accepted: false,
            resume_offset: 0,
        };
        let _ = write_json(&mut send, &response).await;
        return Ok(());
    }
    ...
    let is_resume = !request.force_restart && out_path.exists();
    ...
    if let Err(e) = std::fs::create_dir_all(final_path.parent().unwrap_or(std::path::Path::new("."))) {
        let _ = event_tx.try_send(crate::network::alpn::TransferEvent::Error(TransferErrorEvent { request_id: request.request_id.clone(), error: format!("Cannot create output directory: {e}") }));
        return Err(e.into());
    }
    ...
    let resume_offset = if is_resume {
        let existing_len = std::fs::metadata(&out_path).map(|m| m.len()).unwrap_or(0);
        // Align resume offset to STREAM_CHUNK_SIZE
        (existing_len / STREAM_CHUNK_SIZE as u64) * STREAM_CHUNK_SIZE as u64
    } else {
        0
    };
    ...
    let writer_task = tokio::task::spawn_blocking(move || -> anyhow::Result<String> {
        ...
        while let Some(chunk) = rx_chan.blocking_recv() {
            ...
            writer.write_all(&chunk).map_err(|e| {
                ...
                let _ = event_tx_writer.blocking_send(crate::network::alpn::TransferEvent::Error(TransferErrorEvent { request_id: req_id_for_writer.clone(), error: msg.clone() }));
                anyhow::anyhow!(msg)
            })?;
        }
            ...
            match read_res? {
                ...
                n => {
                    ...
                    if last_progress.elapsed() > progress_interval {
                        let total_received = resume_offset + bytes_received;
                        if let Some(mut state) = ACTIVE_TRANSFERS.get_mut(req_id.as_ref()) {
                            state.bytes_transferred = total_received;
                        }
                        let _ = event_tx.try_send(crate::network::alpn::TransferEvent::Progress(
    ...
    // Surface any error that occurred during receive.
    if let Err(e) = recv_result {
        let _ = std::fs::remove_file(&final_path);
        let err_str = e.to_string();
        if err_str.contains("cancelled") || err_str.contains("Cancelled") {
            tracing::info!(request_id = %req_id, "Transfer cancelled by user");
            let _ = event_tx.try_send(crate::network::alpn::TransferEvent::Cancelled { request_id: req_id.to_string() });
            ACTIVE_TRANSFERS.remove(req_id.as_ref());
            return Err(e);
        }
        let msg = format!("Network error during receive: {e}");
        tracing::error!(request_id = %req_id, "{msg}");
        let _ = event_tx.try_send(crate::network::alpn::TransferEvent::Error(TransferErrorEvent { request_id: req_id.to_string(), error: msg.clone() }));
        ACTIVE_TRANSFERS.remove(req_id.as_ref());
        return Err(e);
    }
    ...
    let _ = event_tx.try_send(crate::network::alpn::TransferEvent::Complete(
...
pub async fn send_file_to_peer(
    ...
    // ── Phase: Connecting ────────────────────────────────
    emit_send_status(&event_tx, &req_id, "connecting");
    ...
    let connection = tokio::time::timeout(
        ...
        emit_transfer_error(event_tx, &req_id, &msg);
        anyhow::anyhow!(msg)
    })?.map_err(|e| {
    ...
    emit_send_status(&event_tx, &req_id, "waiting_for_accept");
    ...
    if !response.accepted {
        ...
        let _ = event_tx.try_send(crate::network::alpn::TransferEvent::Rejected(TransferRejectedEvent {
            request_id: req_id.to_string(),
        }));
    ...
    emit_send_status(&event_tx, &req_id, "accepted");
    ...
    let reader_task = tokio::task::spawn_blocking(move || -> anyhow::Result<String> {
        ...
        let mut file = std::fs::File::open(&file_path_clone).map_err(|e| {
            ...
            let _ = event_tx_reader.blocking_send(crate::network::alpn::TransferEvent::Error(TransferErrorEvent { request_id: req_id_for_reader.to_string(), error: msg.clone() }));
            anyhow::anyhow!(msg)
        })?;
    ...
    // ── Phase: Transferring ────────────────────────────────
    emit_send_status(&event_tx, &req_id, "transferring");
    ...
    loop {
        ...
        if last_progress.elapsed() > progress_interval {
            let total_sent = start_offset + bytes_sent;
            if let Some(mut state) = ACTIVE_TRANSFERS.get_mut(req_id.as_ref()) {
                state.bytes_transferred = total_sent;
            }
            let _ = event_tx.try_send(crate::network::alpn::TransferEvent::Progress(
                TransferProgressEvent {
                    request_id: req_id.to_string(),
    ...
    emit_send_status(&event_tx, &req_id, "done");
    ACTIVE_TRANSFERS.remove(req_id.as_ref());
    Ok(hash)
}
...
pub enum TransferEvent {
    SendStatus(SendStatusEvent),
    Rejected(TransferRejectedEvent),
...
fn emit_send_status(event_tx: &tokio::sync::mpsc::Sender<TransferEvent>, request_id: &str, status: &str) {
    if let Some(mut state) = ACTIVE_TRANSFERS.get_mut(request_id) {
        state.status = status.to_string();
    }
    let _ = event_tx.try_send(TransferEvent::SendStatus(SendStatusEvent {
        request_id: request_id.to_string(),
        status: status.to_string(),
    }));
}

pub fn emit_transfer_error(event_tx: &tokio::sync::mpsc::Sender<TransferEvent>, request_id: &str, error: &str) {
    if let Some(mut state) = ACTIVE_TRANSFERS.get_mut(request_id) {
        state.status = "error".to_string();
    }
    let _ = event_tx.try_send(TransferEvent::Error(TransferErrorEvent {
        request_id: request_id.to_string(),
